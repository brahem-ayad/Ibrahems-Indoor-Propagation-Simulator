#include"../include/Viewport_Gizmo.hpp"
...
void Draw_Viewport_Gizmo() {
  Vector2 Position = {(float)GetScreenWidth() - 70, 22 + 10 + 50}; // 22 is the main menu bar height. 10 is a margin. and 50 is the distance to the center of the gizmo's circle

  Color Inner_Circle_Color;
  Color Outer_Circle_and_Triangles_Color;
  Color Text_Color;
    Inner_Circle_Color = {200, 200, 200, 255};
    Outer_Circle_and_Triangles_Color = {124, 124, 124, 255};
    Text_Color = BLACK;
  ...
  Vector2 text_size = MeasureTextEx(CONF::Montserrat_Font_32, text, 32, 0);
  DrawTextEx(CONF::Montserrat_Font_32, text, {Position.x - text_size.x/2, Position.y - text_size.y/2}, 32, 0, Text_Color);
  ...
  if(CheckCollisionPointTriangle(GetMousePosition(), V1, V2, V3)){
    ...
    if(IsMouseButtonDown(MOUSE_LEFT_BUTTON)){
      if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) Rotate_3D_Camera_Around_Target_UP();
      else if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Movement_Mode) { Move_3D_Camera_UP(); CAMERA::is_moving = true; }
      else CONF::camera2.target.y -= CAMERA::speed * 5;
    }
  }
  // RIGHT Triangle
  ...
  if(CheckCollisionPointTriangle(GetMousePosition(), V4, V5, V6)){
    ...
    if(IsMouseButtonDown(MOUSE_LEFT_BUTTON)){
      if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) Rotate_3D_Camera_Around_Target_RIGHT();
      else if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Movement_Mode) { Move_3D_Camera_RIGHT(); CAMERA::is_moving = true; }
      else CONF::camera2.target.x += CAMERA::speed * 5;
    }
  }
  // LEFT Triangle
  ...
  if(CheckCollisionPointTriangle(GetMousePosition(), V7, V8, V9)){
    ...
    if(IsMouseButtonDown(MOUSE_LEFT_BUTTON)){
      if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) Rotate_3D_Camera_Around_Target_LEFT();
      else if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Movement_Mode) { Move_3D_Camera_LEFT(); CAMERA::is_moving = true; }
      else CONF::camera2.target.x -= CAMERA::speed * 5;
    }
  }
  // DOWN Triangle
  ...
  if(CheckCollisionPointTriangle(GetMousePosition(), V10, V11, V12)){
    ...
    if(IsMouseButtonDown(MOUSE_LEFT_BUTTON)){
      if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) Rotate_3D_Camera_Around_Target_DOWN();
      else if(CONF::View == View_3D and CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Movement_Mode) { Move_3D_Camera_DOWN(); CAMERA::is_moving = true; }
      else CONF::camera2.target.y += CAMERA::speed * 5;
    }
  }
  ...
  if(CONF::View == View_3D){
    ...
    char* text_R_or_M;
    if(CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) text_R_or_M = (char*)"R";
    else text_R_or_M = (char*)"M";
    Vector2 text_R_or_M_Size = MeasureTextEx(CONF::Montserrat_Font_32, text_R_or_M, 32, 0);
    ...
    DrawTextEx(CONF::Montserrat_Font_32, text_R_or_M, text_R_or_M_Pos, 32, 0, Text_Color);
    if(CheckCollisionPointCircle(GetMousePosition(), Rotate_or_Move_with_Arrows_Switch, 15)){
      ...
      if(IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
        if(CONF::Gizmo_Arrows_3D_Mode == Gizmo_Arrows_Rotation_Mode) CONF::Gizmo_Arrows_3D_Mode = Gizmo_Arrows_Movement_Mode;
        else CONF::Gizmo_Arrows_3D_Mode = Gizmo_Arrows_Rotation_Mode;
      }
    }
  }
  ...
  if(CONF::View == View_3D){
    ...
    Vector2 Camera_Projection_Switch_Text_Size = MeasureTextEx(CONF::Montserrat_Font_32, Camera_Projection_Switch_Text, 32, 0);
    ...
    DrawTextEx(CONF::Montserrat_Font_32, Camera_Projection_Switch_Text, Camera_Projection_Switch_Text_Pos, 32, 0, Text_Color);
  ...
  if(CONF::View == View_2D and CONF::Mode == Drawing_Mode){
    Vector2 Snapping_Toggle_Position = {Position.x + 35, Position.y + 35 + 35};
    if(CONF::Enable_Snapping) {
      DrawCircleV(Snapping_Toggle_Position, 18, {51, 148, 217, 255});
      DrawCircleV(Snapping_Toggle_Position, 15, SKYBLUE);
    }
    else {
      DrawCircleV(Snapping_Toggle_Position, 18, Outer_Circle_and_Triangles_Color);
      DrawCircleV(Snapping_Toggle_Position, 15, Inner_Circle_Color);
    }
    DrawTexture(CONF::Magnet_Icon, Snapping_Toggle_Position.x - 15, Snapping_Toggle_Position.y - 15, WHITE);
    if(CheckCollisionPointCircle(GetMousePosition(), Snapping_Toggle_Position, 15)){
      DrawCircleV(Snapping_Toggle_Position, 15, Fade(WHITE, 0.2));
      if(IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
        CONF::Enable_Snapping = !CONF::Enable_Snapping;
      }
    }
  }
